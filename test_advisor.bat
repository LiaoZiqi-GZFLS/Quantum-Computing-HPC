...
if /i "%input%" == "y" (
	plink -ssh -pw %password% -P %port% %user%@%sever%  -batch "advisor --collect=survey --project-dir=%remotePath%/adv_results -- ./auto_submit.sh ./auto_run.sh > advisor_log.txt 2>&1"
    ...
    plink -ssh -pw %password% -P %port% %user%@%sever%  -batch "advisor --report=survey --project-dir=%remotePath%/adv_results --report-output=%remotePath%/survey_report.html"
    pscp -ssh -pw %password% -P %port% %user%@%sever%:"%remotePath%/advisor_log.txt" ./
    pscp -ssh -pw %password% -P %port% %user%@%sever%:"%remotePath%/survey_report.html" ./
) else (
    echo Testing...
